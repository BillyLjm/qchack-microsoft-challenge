namespace QCHack.Task2 {
    ...
    operation Task2_ValidTriangle (inputs : Qubit[], output : Qubit) : Unit is Adj+Ctl {
        use ancillae = Qubit[2];
        within {
            CNOT(inputs[0], ancillae[0]);
            CNOT(inputs[1], ancillae[0]);
            CNOT(inputs[1], ancillae[1]);
            CNOT(inputs[2], ancillae[1]);
            X(ancillae[0]);
            X(ancillae[1]);
        } apply {
            CCNOT(ancillae[0], ancillae[1], output);
            X(output);
        }
    }
}
